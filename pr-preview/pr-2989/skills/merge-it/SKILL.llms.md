...
- Then record that PR’s `headRefOid` before the readiness check below, so its result is tied to one SHA, and require the live head to equal it immediately before every direct merge, on the initially current path as much as after a recovery.
