...
Package logos are commonly written as HTML `<img>` tags at the top of `README.Rmd`.
Check that the `alt` attribute is present and descriptive:
...
For purely decorative images, leave the bracket empty intentionally and add a comment:
...
For the home page, `build_site()` will warn if any README images are still missing alt text.
