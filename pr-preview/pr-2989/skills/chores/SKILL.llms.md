...
Under a queue that command returns when the PR is enqueued, not when it merges. `state` alone cannot tell a waiting PR from an evicted one, since both read `OPEN`. Poll the GraphQL `state`, `isInMergeQueue`, and `mergeQueueEntry` fields (the query in [`merge-it`](../../skills/merge-it/SKILL.llms.md) step 3) until `state` is `MERGED` before counting the bump in the Merged column, and report `OPEN` with `isInMergeQueue` false and a null `mergeQueueEntry`, read after the enqueue succeeded, as an eviction, which is a failed merge.
