#!/usr/bin/env python3
"""Tests for scripts/check-links.py (ai-config#2530).

Validates that check-links.py correctly recognizes autolinks (<https://...>,
<mailto:...>) inside nested markdown list items, sublists, and prose,
...
def main() -> None:
    ...
    # Autolinks in markdown lists and nested sublists
    doc_nested_lists = """
# Nested List Fixture

- Root item 1
  - Sublist item with autolink <https://github.com/Morrison-Lab/ai-config>
  - Sublist item with mailto autolink <mailto:support@example.org>
  - Sublist item with relative link [architecture](docs/arch.md)
    * Deep sublist level 3 with autolink <https://developer.mozilla.org/en-US/docs/Web>
    * Deep sublist with email autolink <developer@example.org>
    1. Numbered sublist level 4 with autolink <https://python.org>
    2. Numbered sublist with relative link [changelog](CHANGELOG.md)
+ Plus-bullet item with <https://example.com/plus>
  + Nested plus-bullet item with <https://example.com/plus-nested>
- [ ] Task list unchecked with <https://example.com/task1>
- [x] Task list checked with <https://example.com/task2>
"""
    nested_targets = check_links.extract_targets(doc_nested_lists)
    expected_nested = [
        ...
        "https://example.com/plus-nested",
        "https://example.com/task1",
        "https://example.com/task2",
    ]
    for expected in expected_nested:
        check(f"extract_targets: nested list item contains '{expected}'", expected in nested_targets)
...
```markdown
- List in code block <https://code-example.com>
...
"""
    ...
    doc_inline_code = "Use `<https://inline-code-example.com>` to configure the endpoint."
    check(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Create valid markdown file containing nested lists with autolinks and valid relative links
        test_file = tmppath / "README.md"
        test_file.write_text(doc_nested_lists, encoding="utf-8")

        broken = check_links.check_file(test_file)
        check("check_file: nested lists with autolinks report 0 broken links", len(broken) == 0)

        # Create file with broken relative link in a sublist
        broken_doc = """
- Root
  - Sublist item with <https://example.com>
  - Sublist item with broken link [missing](docs/missing.md)
"""
        ...
        check("check_file: detects broken relative link in sublist", len(broken_results) == 1)
