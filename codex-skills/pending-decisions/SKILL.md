...
The source lives at `skills/pending-decisions/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/pending-decisions`, resolve the symlink target for this wrapper directory first, then read `../../skills/pending-decisions/SKILL.md` relative to that real directory.
