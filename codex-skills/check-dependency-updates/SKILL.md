...
The source lives at `skills/check-dependency-updates/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/check-dependency-updates`, resolve the symlink target for this wrapper directory first, then read `../../skills/check-dependency-updates/SKILL.md` relative to that real directory.
