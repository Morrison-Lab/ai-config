...
A `stale info` refusal is not a reason to force either: `memories/git.md` records that it means the remote branch is gone.
If this session already had a PR on that branch, read the PR's `state` before a plain push.
MERGED or CLOSED means do not recreate the deleted head.
