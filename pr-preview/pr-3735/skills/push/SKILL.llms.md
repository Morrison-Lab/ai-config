...
- **`no-push-without-self-review.py`**: Blocks `git push` unless a clean verdict for the exact commit being pushed (`Reviewed-Commit: <HEAD_SHA>`) came from an adversarial self-review subagent, or from a review by a CLI the guard recognizes, which today means `agy --print` and no other. Dispatch the reviewer in the foreground and resolve all findings before pushing. Use `ALLOW_UNREVIEWED_PUSH=1` only for the empty `pr-on-claim` commit or documented exceptions.
