...
| `flag-unmeasured-digest.py` | `PreToolUse` (Bash, mcp__github__.*) | warns, never blocks, when a body about to leave through `gh`/`glab` (a comment, or an issue or PR being created or edited) or an MCP comment tool cites a digest-shaped token -- 7 to 64 hex characters with at least one `a`-`f`, that is a canonical length (32/40/64), or truncated with an ellipsis, or preceded within 48 characters by a digest word -- and no prior tool result or user message in the transcript contains a hex run having that token as a prefix, so a legitimate abbreviation of a measured hash stays quiet while an invented one does not (ai-config#3779) |
