...
- **An empty BALANCED "Needs a closer look" review is non-blocking; the identical review at LITE effort still blocks.**
  ...
  Tested in `scripts/test_enforce_mwc_review_gate.py` against the real, verbatim body of Lacaedemon/sparta#1638's review 5316721676 (and the same body under CRLF), which the template must always match, alongside every adversarial probe that defeated an earlier design and a harmless trailing "Resolved since last review" block that the template still refuses.
