...
### 0b. The CI validators pass locally

Run the derived pre-push suite, and read its denominator line:
...
A step listed as **not runnable locally** (a `uses:`-only job with no local
equivalent, or a step reading a `${{ github.* }}` expression) is a check CI
will run that this pass did not; say so when you report the push as
pre-validated.
