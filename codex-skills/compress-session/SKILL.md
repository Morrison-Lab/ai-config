...
The source lives at `skills/compress-session/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/compress-session`, resolve the symlink target for this wrapper directory first, then read `../../skills/compress-session/SKILL.md` relative to that real directory.
