...
## ai-config's own review is dispatch-only, and repeated dispatches cancel each other

The section above notes in passing that ai-config's bot uses different filenames.
It also differs in a way that changes what a session has to do, and in a second way that makes a chain of cancelled runs easy to misdiagnose.

**`.github/workflows/claude-review.yml` here carries `workflow_dispatch` and nothing else.**
There is no `pull_request` trigger, so opening a PR or pushing to one fires no review at all.
Every other statement in this file presumes both triggers are present and that a push auto-fires a fresh review, which is true of the Quarto and R-package repos those bullets came from and is not true here.
So a session waiting for a review to appear on a fresh ai-config PR waits forever.
Read the `on:` block rather than assuming, and dispatch it:
...
The group lives in the reusable workflow it invokes, `Morrison-Lab/gha/.github/workflows/claude-code-review.yml@v2`:
...
So every dispatch for one PR lands in a single group and cancels its still-running predecessor, whatever `--ref` each was given, and only the last survives.
An absent `concurrency:` block in the caller is therefore not evidence that a dispatched run cannot be cancelled.

**The trap is attribution, and it is the reason this is worth recording.**
...
Any variable at all would look causal under this design, so the retry that worked is the weakest possible evidence about why it worked.

Measured on PR #1224, 2026-08-06, all four runs confirmed `PR_NUMBER: 1224` from their own `gather-context` job logs:
...
The second row settles it: a run dispatched **with** `--ref <pr-branch>` was cancelled too, by a later run dispatched **without** it.
...
Pass `--ref` anyway, for the unrelated reason the section above already gives: `workflow_dispatch` runs the workflow file from the ref you name, defaulting to the default branch.
Just do not read a run's survival as evidence that you passed it.

- **Do:** read the `on:` block before waiting for a review, and dispatch it yourself when `pull_request` is absent.
- **Do:** look for `concurrency:` in the reusable workflow a caller invokes, not only in the caller.
- **Do:** treat "the last dispatch is the one that lived" as the default explanation for a chain of cancelled review runs, checking dispatch timestamps against cancellation timestamps before reaching for anything else.
