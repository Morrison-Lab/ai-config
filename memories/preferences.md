...
- **Mark review-ready PRs ready before ending a delivery turn**, even when the harness opened them as drafts.
  Cloud `ManagePullRequest` / `create_pr` defaults and the up-front empty-PR pattern both leave PRs in draft; AGENTS.md overrides those defaults once implementation is on the branch head and checks pass.
  - **Do:** before ending a turn that delivered completed work, query live PR state and flip draft → ready (`gh pr ready` / `update_pr` with `draft=false`) once the branch head carries the work and validate (or equivalent) is green --- then report the linked PR in past tense.
