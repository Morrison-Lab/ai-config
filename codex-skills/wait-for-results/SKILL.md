...
The source lives at `skills/wait-for-results/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/wait-for-results`, resolve the symlink target for this wrapper directory first, then read `../../skills/wait-for-results/SKILL.md` relative to that real directory.
