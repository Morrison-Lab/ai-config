...
- **An empty BALANCED "Needs a closer look" review is non-blocking; the identical review at LITE effort still blocks.**
  ...
  That still wasn't the whole fix: the same function read the heading, the effort lines, and the findings lines anywhere in `live_body`, with no requirement that they belong to the SAME overview.
  A reviewer reproduced it directly: a live `Needs a closer look` heading with plain prose under it, then a `<!-- ccr-overview-v2 -->` marker, then a genuine but heading-LESS `## Copilot review overview` block carrying `**Review effort:** Balanced` and `**Findings:** None`.
  The gate carved this out as no verdict; `check-pr-fully-clean.py`'s `copilot_verdict()` correctly read it as `not-clean`, since its own block detection never lets a heading before the marker count as part of the block after it.
  Fixed by scoping the gate to one block too: `copilot_is_empty_balanced_closer_look()` now requires EXACTLY ONE `COPILOT_OVERVIEW_MARKER` in the raw body (zero or more than one both fail closed), requires that one marker to be LIVE (`_copilot_marker_is_live()` -- not itself inside another comment or a `<details>` region, checked over the WHOLE unsplit body since a wrapper spanning across the marker is invisible to a before/after split done first), takes the block as everything after that marker, and requires the heading plus a live Balanced line plus a live None line to exist WITHIN that block's own live text -- while also requiring the text BEFORE the marker to carry no live heading and no live effort/findings line at all, so a heading before the marker can never combine with an unrelated block after it.
  The uniformity checks (every effort line found anywhere reads Balanced, every findings line found anywhere reads None) and the `Previously missed`/`Open (N)`/`Changes recommended` guards are unchanged: they still scan the raw, unstripped WHOLE body.
  Covered by three more tests in `scripts/test_enforce_mwc_review_gate.py`: the reviewer's own reproduction denies, two markers in one body denies, and a heading before the marker with nothing at all after it (an empty block) denies.
  ...
  - **Do:** scope that existence check to ONE overview block (found via its own start marker), not to the whole body -- a heading and a Balanced/None pair that merely co-occur somewhere in the body, without belonging to the same round's overview, must not satisfy the carve-out.
