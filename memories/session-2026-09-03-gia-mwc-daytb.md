...
and the same `git log` with `-m` printed 2 in that repo (3 in the reproduction recorded in `memories/git-diffing.md`, whose merge carries two copies against one parent).
...
Promoted to `memories/git-diffing.md` ("A merge commit's own content is visible to `git diff A...B` and invisible to `git log -p`") and `memories/git.md` (the `refs/pull/NNNN/head` classifier).
