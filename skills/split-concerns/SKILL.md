...
- CI/workflow change + heavy simulation/validation dataset artifacts (`inst/extdata/*.rds`, `*.parquet`, `*.RData`) → split
- CI/workflow change + HPC job array updates → split
- Heavy simulation/validation dataset artifacts + HPC job array updates → split
