...
1. `ai-session.sh enable-mwc` creates a `<sanitized-session-id>.mwc` marker file in the repository's git common directory (`$(git rev-parse --git-common-dir)/ai-sessions/`).
2. `no-unauthorized-merge.py` checks for the active session's `.mwc` marker file and validates that the session is alive (`is_session_alive()`).
