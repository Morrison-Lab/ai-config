...
- **Anchor `pgrep -f bwhen unavoidable**:
...
- **Never interpolate shell vars into a `python3 -c / awk program string.**
  Pass them as arguments: `python3 -c '...def main(...)' "$val"` (not `"...'$val'..."`)
...
- **bash 3.2 (macOS default) compatibility:** indexed arrays, C-style
  ...
  Parse key=value records with `while IFS='foo' read -r k v; do case "$k" in ...`.
