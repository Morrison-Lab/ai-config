...
The source lives at `skills/prefer-upstream/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/prefer-upstream`, resolve the symlink target for this wrapper directory first, then read `../../skills/prefer-upstream/SKILL.md` relative to that real directory.
