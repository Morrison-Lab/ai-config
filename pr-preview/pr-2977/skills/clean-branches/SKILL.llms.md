...
`-d` (never `-D`) is deliberate, and its refusal has two readings. `error: the branch 'X' is not fully merged` means unmerged commits, so treat the branch as **stale**, not dead (see b). `error: cannot delete branch 'X' used by worktree at ...` means the branch is checked out in a linked worktree, so leave it alone, give it the status `checked out in a worktree` in the step 4 plan table, and count it under “Skipped” in the step 9 report; `clean-worktrees` is the skill that retires the worktree first.
