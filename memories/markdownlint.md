...
- **One sentence per line and gha's list-item-splice check collide inside a list, and the check names the item you did not touch.**
  ...
  **Candidate check.**
  Per the `markdownlint-cli2 runs locally` entry above, `scripts/run-local-validation.py` offers no local equivalent for this companion, so it reaches a session only as a red CI job.
