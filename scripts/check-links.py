...
deleted target).
Supports inline markdown links ([text](url)) and CommonMark link reference
definitions ([label]: url), including indented destinations on the following line.
...
def _check_target(target: str, md: Path, root: Path = ROOT) -> None:
    ...
    if target.startswith("<"):
        ...
        if end_bracket != -1:
            target = target[1:end_bracket].strip()
        else:
