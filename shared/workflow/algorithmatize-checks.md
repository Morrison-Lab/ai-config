...
Four rounds on one such regex, each wrong in a different direction (`Morrison-Lab/gha#857`, deciding whether a `### Verdict` heading qualifies an existing statement or supersedes it): a bare word-boundary prefix matched `### Verdict rationale`, an ordinary section title, and misfired on every uncorrected review that happened to use the word.
