...
The source lives at `skills/consolidate-memory/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/consolidate-memory`, resolve the symlink target for this wrapper directory first, then read `../../skills/consolidate-memory/SKILL.md` relative to that real directory.
