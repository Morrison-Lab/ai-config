...
(`the repository owner/altdoc#20` → `#22`, 2026-07-14: `#20` compared `etiennebacher/altdoc:main` against the fork's `main` and hit a real `NEWS.md`/`tests/testthat/helper.R` conflict with no push access to fix it on that PR;
...
(Found by hand on `the repository owner/rme#706`: a "conditional predicted risk" quantity introduced only as plain prose right before two definitions that depended on it, and a "collapsibility bias" concept defined in one sentence crammed inside a *different* concept's definition div.)

## Detect hypothetical examples where real data is already available

(Found by hand on `the repository owner/rme#706`: a logistic-regression chapter's worked examples used invented covariate-specific risks and made-up exposure proportions throughout, even though the chapter already loads and fits models on its real running WCGS dataset elsewhere.)
