...
The source lives at `skills/promote-memory/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/promote-memory`, resolve the symlink target for this wrapper directory first, then read `../../skills/promote-memory/SKILL.md` relative to that real directory.
