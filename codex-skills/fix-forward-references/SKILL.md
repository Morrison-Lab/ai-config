...
The source lives at `skills/fix-forward-references/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/fix-forward-references`, resolve the symlink target for this wrapper directory first, then read `../../skills/fix-forward-references/SKILL.md` relative to that real directory.
