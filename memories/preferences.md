...
- **Do not request Copilot code review on any PR, in any repo, until September 2026.**
  ...
  It outranks `hooks/no-unreviewed-pr.py` and `shared/workflow/pr-on-claim.md`'s request-the-reviewer step; state the directive as the reason when a PR ships without one, and re-verify at the expiry.
