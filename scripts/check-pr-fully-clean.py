...
RESOLVED_BLOCKING_SUFFIX = re.compile(
    ...
    r"suppress(?:ed|ing)?|disabl(?:e|ed|ing)|mut(?:e|ed|ing)|weaken(?:ed|ing)?|bypass(?:ed|ing)?|"
    r"comment(?:ed|ing)?\s+out|"
    r"(?:remov|delet|revert)(?:e|ed|ing)?\s+(?:the\s+|an?\s+)?(?:test|check|assertion|warning|linter|guard|lint))\b)"
    r"(?:\([^()\n]{0,120}\)|[^;:,.!?()]|\.(?!\s|$))){1,180})?"
    r"(?:"
