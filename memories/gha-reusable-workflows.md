...
- **`@v1` can trail `main` in practice --- verify against the TAGGED file, not `main` or `examples/`.**
  ...
  Filed as gha#179; worked around in `Morrison-Lab/altdoc`#14 by omitting `ANTHROPIC_API_KEY` until `@v1` catches up.
...
- **The gha family has THREE different R-dependency mechanisms, with opposite defaults, and picking the wrong one fails only at render time.**
  ...
  (Measured 2026-08-24 migrating [Morrison-Lab/macros#83](https://github.com/Morrison-Lab/macros/pull/83), a Quarto site with no `renv.lock` and no `DESCRIPTION` whose `macros-table.qmd` needs `knitr`, `rmarkdown`, and `DT`.)
