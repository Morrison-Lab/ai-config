...
- **Not destructive or irreversible actions.**
  ...
  **Local git housekeeping is the exception, and it is inside the grant.**
  ...
  - **Deleting a local branch** needs its content already on the default branch.
    ...
    Restricting the diff to the branch's own files does not resolve the non-empty case, since a sibling PR that touched the same file reproduces the same confusion.
    An **empty** scoped result is still conclusive, though, provided the file list is a complete enumeration of every path the branch touched (renames and deletions included): if every one of those files matches the default branch byte-for-byte, nothing on the branch is missing from it, whoever wrote the matching content.
    [`pr-on-claim`](../../shared/workflow/pr-on-claim.md)'s "One reading it does not cover" section carries the measured cases: the two-dot diff running to thousands of lines purely because the default branch had since taken later, unrelated merges, and a separate occurrence where scoping that diff to the branch's own files came back empty and correctly confirmed the merge.
