...
2. Local-only daemon configuration (OLLAMA_NO_CLOUD=1 or ~/.ollama/server.json disable_ollama_cloud).
...
def verify_locality(
    ...
    server_json_path: Optional[str] = None,
) -> Tuple[bool, str]:
    ...
    # 3. Verify daemon-side local-only mode
    # Ollama requires OLLAMA_NO_CLOUD=1 or ~/.ollama/server.json with disable_ollama_cloud: true
    server_cfg_file = server_json_path or os.path.expanduser("~/.ollama/server.json")
    ...
    env_cloud_disabled = (os.environ.get("OLLAMA_NO_CLOUD") == "1")

    if not (server_cloud_disabled or env_cloud_disabled):
        return False, "Ollama local-only mode not verified (requires OLLAMA_NO_CLOUD=1 in daemon environment or 'disable_ollama_cloud: true' in ~/.ollama/server.json)."

    # 4. Verify local model residency via /api/tags
    try:
        parsed_url = urlparse(url)
        base_path = re.sub(r"/v1/?$", "", parsed_url.path).rstrip("/")
        tags_url = f"{parsed_url.scheme}://{parsed_url.netloc}{base_path}/api/tags"
