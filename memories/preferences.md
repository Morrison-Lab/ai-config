...
- Keep PRs focused on a single concern: never mix CI/workflow infrastructure changes (`.github/workflows/`) with heavy simulation/validation dataset artifacts (e.g. `inst/extdata/*.rds`, `.parquet`, `.RData`) or HPC job array updates in the same PR --- open dedicated PRs per concern (see `split-concerns`).
