...
What a remote or web session can reach on GitHub from bash when
`gh`/`glab` are absent: session-scope 403s, the
MCP-then-`add_repo`-then-`git ls-remote` ladder, the proxy's
push-but-not-delete rule, and GitHub Pages policy denials.
...
- There is no `gh`/`glab` CLI in these sessions, so `mcp__github__*` is the
  ...
  - **The `github.com` web host 403s on scope exactly as `api.github.com`
    ...
    than the `raw.githubusercontent.com` probe in [`github.md`](github.md)'s
