...
- **MD056 counts a literal `|` inside a code span as a cell delimiter -- backticks do not protect it.**
  ai-config#2906 went red on `lint-markdown` because two hook-catalog rows contained `` `HH:MM PDT|PST|PT` `` (a regex-style alternation written in backticks): `memories/hooks.md:69` reported "Expected: 5; Actual: 7" and `README.md:405` "Expected: 3; Actual: 5".
