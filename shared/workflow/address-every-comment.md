...
[`address-every-comment.rationale.md`](address-every-comment.rationale.md),
...
The count does license a changed *procedure*, though never a stop --- see the
[`ardi` fragment](ardi.md)'s "Three or more review rounds that each returned
findings".
...
**The same reviewer's case, never run at all: a fix verified against your own
RESTATEMENT of a finding is not a fix verified against the finding.**

The section above governs a reviewer's demonstration that *errors* at your
HEAD.
This one governs the demonstration that would have run fine, and simply never
entered your check --- because understanding a report means restating it, and
the restatement is what your test ends up encoding.

The restatement is generated by the same reading that will produce the fix, so
the two agree by construction.
A test built from it cannot fail on the reported case, since the reported case
is not in it, and nothing about a green run announces which input it omitted.

What makes this read as a completed Address rather than a shortcut is that
every artifact a real fix produces is present.
A genuine defect was found on the flagged line.
It was fixed, with tests, and the tests pass.
The diff shows the very line the reviewer pointed at, visibly changed.
The one absent thing is the reviewer's own input, and no check anybody runs
takes it as an argument.

**Fixing a different defect on the flagged line is the specific trap**, and the
shared line is what supplies the false confirmation.
A reviewer points at a predicate;
reading it closely turns up a real bug in it;
fixing that bug changes the predicate and closes nothing the reviewer raised.
Proximity is not identity, and the diff cannot tell them apart --- see
[`verify-the-right-artifact`](verify-the-right-artifact.md), of which this is
the review-round instance: your restatement is the adjacent artifact standing
in for the report.

The remedy is mechanical and costs one command.
Before writing "Addressed", paste the reviewer's **literal** input into a test
or a shell invocation and run it.
A finding that quotes a concrete value is handing you its own acceptance
criterion, so use that value rather than one shaped like it.
Where the report quotes no input, derive the discriminator instead: name what
the reviewer's case would show if your fix were incomplete, per
[`ardi`](ardi.md)'s "name the observation that would differ if your change were
the cause".

- **Do:** run the reviewer's quoted input, unmodified, against the fixed code
  before reporting the finding Addressed.
- **Do:** paste that input and its post-fix output into the reply, so the
  disposition carries its own evidence rather than your reading of the report.
- **Do:** ask whether the bug you fixed is the bug that was reported, whenever
