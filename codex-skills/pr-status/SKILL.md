...
The source lives at `skills/pr-status/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/pr-status`, resolve the symlink target for this wrapper directory first, then read `../../skills/pr-status/SKILL.md` relative to that real directory.
