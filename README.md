...
| `flag-self-authored-verdict-echo.py` | `PreToolUse` (Bash, mcp__github__.*) | warns, never blocks, when a forge comment about to post or be edited classifies as a NOT-CLEAN verdict by `scripts/check-pr-fully-clean.py`'s own `classify_verdict()` while carrying ARD disposition vocabulary and no `review-data:` payload --- the shape of a reply that ANSWERS findings rather than a review that states them, which on Morrison-Lab/mln#49 gave its own author a standing not-clean verdict (unsupersedable per ai-config#2274) on a head with 14 check runs green and a live clean review verdict; blockquoting the echoed line and wrapping it in a backtick code span were both measured and neither exempts it, so the message says to describe the call rather than reproduce it |
