...
PR #1223 --- open at the time of writing, so confirm it merged before relying
on the citation --- adds a "Check the GitHub status page when workflows stall
across several PRs at once" section covering **detection**: absent checks
across several unrelated PRs, the `githubstatus.com` JSON endpoints, and the
plural-versus-single tell.
This section is the other half, and it starts where that one stops: the
incident has resolved, and the PRs it hit are still broken in two different
ways that call for two different fixes.
