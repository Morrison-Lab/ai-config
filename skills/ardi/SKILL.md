...
Request `d-morrison` via the `request-pr-review` skill (or `gh pr edit <N> --add-reviewer d-morrison`; on `Morrison-Lab/ai-config` the skill requests nobody and the deadlock goes to the user in chat as a boxed `🛑 BLOCKER` instead), `@`-mention them in a comment summarizing the impasse, and surface the open item to the user.
