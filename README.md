...
| `no-delete-branch-under-stacked-pr.py` | `PreToolUse` (Bash) | warns when `gh pr merge --delete-branch` would delete a branch that is an open PR's base. GitHub closes such a PR rather than retargeting it, and a closed PR can be neither retargeted nor reopened while its base is gone. Silent when nothing is stacked, when the query fails, or when `gh` is absent |
