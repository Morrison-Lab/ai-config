...
The source lives at `skills/check-info-quality/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/check-info-quality`, resolve the symlink target for this wrapper directory first, then read `../../skills/check-info-quality/SKILL.md` relative to that real directory.
