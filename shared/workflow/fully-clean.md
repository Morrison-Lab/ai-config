...
- **Do:** when the merge-base is not that tip and the merge is direct, update the branch pinned to the recorded head (the `expected_head_sha` update call, or `update_pull_request_branch` with `expectedHeadSha` remotely), then rerun the whole clean gate on the new head, review included, before merging.
  ...
  A `422` whose message names an expected-head mismatch means the head already moved --- but only when the live head actually has.
  ...
  That message text does not by itself discriminate a moved head from a wrong pin, so re-read `headRefOid` and compare it to the SHA you sent before routing to the ownership rule (settle who owns the branch per [`claim-pr`](claim-pr.md)) instead of merging the base into someone else's push.
  A correctly-lengthed but wrong-content SHA --- most often one guessed or padded from an abbreviation instead of read in full --- produces the byte-identical `422` with no other writer involved, so treating the message alone as the another-writer signal sends a self-inflicted bad pin into a concurrent-writer investigation instead of the one-command fix (re-read the real SHA and retry).
