...
- **The reviewer-request API is not the surface to check, and a `422` reported for it did not reproduce.**
  ...
  **As of 2026-08-04, Copilot is quota-exhausted across Morrison-Lab, so do not request it at all while the moratorium stands.**
  Its live expiry is `MORATORIUM_END` in `hooks/no-unreviewed-pr.py`, extended to December 2026 on 2026-09-02.
  ...
  Until `MORATORIUM_END`, rely on `claude-review` plus self-review, which is exactly the no-reachable-external-reviewer fallback that fifth case already describes.
  **Measured 2026-09-01, then superseded the next day.**
  Read the two as a sequence rather than a contradiction.
  The quota did return, and Copilot code reviews resumed across Morrison-Lab in September 2026
...
- **`gh issue comment` on a PR uses GraphQL `addComment`, requiring `pull_requests: write`.**
  ...
  - **Do:** in workflows with `issues: write` commenting on PRs (e.g. comment-triggered dispatchers), use the REST endpoint via `gh api --method POST /repos/{owner}/{repo}/issues/{issue_number}/comments -f body=...` (or pass JSON with `jq -Rs '{body: .}'`).
  - **Don't:** use `gh issue comment` in jobs lacking `pull_requests: write` when commenting on pull requests.
...
- **`gh workflow run` without `--ref` queries GraphQL `repository.defaultBranchRef`, failing when missing `contents: read`.**
  ...
  On private repositories or with limited-permission tokens (such as a GitHub Actions job with only `actions: write`), this query fails with `GraphQL: Resource not accessible by integration: unable to determine default branch`.
