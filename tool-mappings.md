...
| `LABEL_ISSUE` | Set an issue's labels. `--add-label` adds to existing; MCP replaces the whole set, so pass the union. MCP also silently creates unknown labels. | `gh issue edit "<N>" --add-label "..."` | `mcp__github__issue_write (method=update, labels=[...])` |
