...
| [`no-fable-subagent.py`](../hooks/no-fable-subagent.py) | `Agent`, `Task` | Deny / warn | Denies a subagent dispatch that names Fable without `ALLOW_FABLE_SUBAGENT=1`, or that names no `model` while the transcript's last assistant entry shows the session is on Fable (the worker would inherit it); an omitted `model` on a non-Fable or unknown session (the Antigravity adapter's shape) only warns (user directive 2026-09-01, ai-config#2929). | Name `model` on every `Agent` call: `sonnet`/`haiku` for mechanical work, `opus` for judgment-heavy work; never `fable`, named or inherited, unless the user granted that dispatch in their own words, and then set the override and say which words granted it. |
