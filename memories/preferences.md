...
- **ALWAYS run UMS IMMEDIATELY upon any user correction, incorrect claim, or missed item.**
  ...
  **Scrutiny of the work is the same immediate trigger, including a path that never looks like a correction.**
  Run UMS when you read a review of your work, when you receive critical feedback on it, or when a questioned claim ("are you sure about that?") turns out to be wrong.
  Do not wait for Address, a clean verdict, or a first-person admission; answering with the corrected fact is not the pass.
  Questioning alone does not owe a pass when the claim holds.
  (User directive / CAI, 2026-08-25, [ai-config#2261](https://github.com/Morrison-Lab/ai-config/issues/2261).)
