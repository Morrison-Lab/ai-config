...
The source lives at `skills/rescue-closed/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/rescue-closed`, resolve the symlink target for this wrapper directory first, then read `../../skills/rescue-closed/SKILL.md` relative to that real directory.
