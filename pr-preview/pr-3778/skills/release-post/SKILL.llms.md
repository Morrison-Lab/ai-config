Create professional package release blog posts for Tidyverse/Shiny.
...
**For Tidyverse posts**, read [references/tidyverse-formatting.md](references/tidyverse-formatting.md) and apply: - hugodown frontmatter with `slug`, `photo.url`, `photo.author` - Specific slug format: `packagename-x-y-z` (hyphens replace dots) - R code blocks with `r` language identifier - Acknowledgements always included as final section

**For Shiny posts**, read [references/shiny-formatting.md](references/shiny-formatting.md) and apply: - Quarto frontmatter with YAML anchors for social media - Flexible title formatting - Use tabsets for Python/R or Express/Core variations - Platform-specific code block attributes - Acknowledgements optional, varies by post type - May use lead paragraphs, callouts, embedded media
...
**Tidyverse:** - \[ \] Slug format: `package-x-y-z` (hyphens, not dots) - \[ \] Photo URL and author included - \[ \] Acknowledgements section is final section - \[ \] All contributors listed alphabetically

**Shiny:** - \[ \] YAML anchors used for description (`&desc`, `*desc`) - \[ \] Social media cards configured (`open-graph`, `twitter-card`) - \[ \] Appropriate filters specified if using tabsets/shinylive - \[ \] Tabsets used for showing paired variants (Python/R, Express/Core) - \[ \] Multi-language tabsets used consistently (for R+Python packages only)
...
**[references/content-guidelines.md](references/content-guidelines.md)** - General best practices for all release posts: - Post structure and organization - Opening style and tone - Section hierarchy and organization - Code examples and formatting - Before/after patterns - Acknowledgments conventions

### Platform-Specific Formatting

**[references/tidyverse-formatting.md](references/tidyverse-formatting.md)** - Tidyverse blog requirements: - hugodown frontmatter structure - Slug and title conventions - Photo attribution - Code block formatting - Lifecycle section structure - Acknowledgements format

**[references/shiny-formatting.md](references/shiny-formatting.md)** - Shiny blog requirements: - Quarto frontmatter with YAML anchors - Social media card configuration - Lead paragraphs and callouts - Tabsets for variants - Line highlighting and annotations - Video embedding - Flexible acknowledgements
