...
claim-checking fan-out.
You check a single claim, reasoning step, or computed
...
3. **A computed value or figure.**
   ...
   is given, fetch it and read the actual value off the rendered output.
   If no rendered artifact is available, say so explicitly rather than guessing
   from the prose.
