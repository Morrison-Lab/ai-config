...
The source lives at `skills/workaround-watcher/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/workaround-watcher`, resolve the symlink target for this wrapper directory first, then read `../../skills/workaround-watcher/SKILL.md` relative to that real directory.
