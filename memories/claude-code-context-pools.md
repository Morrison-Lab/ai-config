...
So the figure above is not merely large; it exceeds what at least one consumer of it can accept.
...
- **Don't:** propose splitting a large `CLAUDE.md` into `@path` imports as a context saving; it buys organization and nothing else.
...
`shared/workflow/fully-clean.md`, `ardi.md`, and `address-every-comment.md` do NOT carry the marker, despite an earlier version of this entry claiming otherwise; they are large but ai-config-only, so restructuring them (e.g. ai-config#1236's `fully-clean.md` split) needs no manual coordination.
...
This is why a 2026-08 case-record-extraction pass touched only the ai-config-only fragments (those without the marker); the lab-manual-shared ones are tracked in Morrison-Lab/ai-config#1191 (move records out) and #1192 (condense them in place).
...
`scripts/check-context-closure.py`, which implements the closure walk, follows only `@path` imports (whole-line or inline in prose); a `[text](topic.cases.md)` markdown link matches neither, so its target is never followed into the closure.
