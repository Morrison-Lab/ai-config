...
In remote/web sessions, load the merge tool’s schema with `ToolSearch` (`select:mcp__github__merge_pull_request`) before the first call to confirm the exact name and parameters – [`tool-mappings.md`](../../tool-mappings.md) is the canonical `gh`→MCP reference (per `CLAUDE.md`’s “Skills that call gh/glab” rule); `d-morrison/gha`’s own CLAUDE.md carries an equivalent table for that repo.
