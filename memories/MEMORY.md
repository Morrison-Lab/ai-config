...
| [`github-actions.md`](github-actions.md) | GitHub Actions & the `@claude` bot workflows | Workflow authoring gotchas, YAML quoting for Actions, the `d-morrison/gha` reusable workflows, the `@claude` CI action and PR-review workflow (re-triggering, the self-modification guard, `claude-code-action`'s tag vs. agent modes), and gha's changelog conventions. |
...
| [`claude-code.md`](claude-code.md) | Claude Code harness & agent tooling | The harness itself: `AskUserQuestion`, the Bash tool (zsh, cwd persistence), `WebFetch`, `ScheduleWakeup` / `Monitor`, custom subagents, `Workflow` `agent()`, `codex`, skill command blocks, and this repo's own memory-file structure. |
