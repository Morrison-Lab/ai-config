...
Request `d-morrison` via the `request-pr-review` skill, `@`-mention them in a comment summarizing the impasse, and surface the open item to the user.
The raw `gh pr edit <N> --add-reviewer d-morrison` form bypasses that skill and so does **not** inherit its `Lacaedemon/sparta` exception; in sparta, escalate to the user in chat rather than requesting a reviewer at all.
