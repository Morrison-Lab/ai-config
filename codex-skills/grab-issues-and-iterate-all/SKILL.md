...
The source lives at `skills/grab-issues-and-iterate-all/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/grab-issues-and-iterate-all`, resolve the symlink target for this wrapper directory first, then read `../../skills/grab-issues-and-iterate-all/SKILL.md` relative to that real directory.
