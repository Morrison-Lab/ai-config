...
| `flag-unmeasured-timestamp.py` | `PreToolUse` (Bash, mcp__github__.*) | warns, never blocks, when a `gh pr comment`/`gh issue comment`/`gh api .../comments` body or an `mcp__github__add_issue_comment`/`add_reply_to_pull_request_comment` body about to post states a Pacific clock time (an `HH:MM` stamp suffixed `PDT`, `PST`, or `PT`) and no clock read appears in the transcript since the current turn began, or the stamp runs ahead of the harness's injected reading -- the forge-comment half of the timestamp rule, whose chat-recap half is `no-unmeasured-clock-claim.py` at `Stop`; the warning names the stamp and the `TZ=America/Los_Angeles date` command to run before restating it (ai-config#2900's claim comments stamped up to an hour ahead of the clock, #2903) |
