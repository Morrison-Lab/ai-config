...
| **CC-BY-NC / CC-BY-NC-SA** | **Read-only for a permissively licensed repo.** The NonCommercial term does not survive relicensing, so it cannot go into an MIT/BSD/Apache repo at all --- not even prose, not even with attribution. Clean-room the *idea* and credit it as inspiration. *Using* the work is still fine where the use is noncommercial; this row is about copying. |
| **CC-BY-ND** | **Read-only.** No derivatives, so adaptation is out even where copying verbatim would be allowed. Idea only. |
