...
The source lives at `skills/configure-gitattributes/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/configure-gitattributes`, resolve the symlink target for this wrapper directory first, then read `../../skills/configure-gitattributes/SKILL.md` relative to that real directory.
