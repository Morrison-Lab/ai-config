...
>    A region-wide match has one measured false positive: ai-config#1036 review `4837539268`, whose collapsed `Show a summary per file` table reads "Detects suppressed Copilot findings." while the body carries no suppression block (137 Copilot review bodies enumerated 2026-09-04, and the set is in [`fully-clean.cases.md`](../../shared/workflow/fully-clean.cases.md)).
>    Keep it as a fallback behind the heading anchor on the cost asymmetry --- a false zero merges over real findings, a false positive costs one re-read --- and treat a hit only the fallback finds as probably spurious, with a collapsed per-file summary table as the shape to expect when you re-read that region.
...
>    Copilot posts more than one review per head, and each carries its own suppression block --- three at head `6f10014` on ai-config#3084 (`5098574802`, `5098854246`, `5098881593`), each with a `### Suppressed comments (1)` block of its own (measured 2026-09-03), so reducing to one of them scans only that review's block and silently drops the other two's findings --- and reports `clean` outright whenever the review the reduction keeps is the finding-free one.
