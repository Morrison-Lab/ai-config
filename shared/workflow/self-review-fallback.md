...
The pre-flight credential shape check in `Morrison-Lab/gha` (`check-credential-shape`, gha#686) detects when every configured API credential (`CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY`) contains internal whitespace, newlines, or multi-line formatting (such as a pasted PEM block, JSON credential, or wrapped terminal copy).
