...
- **The reviewer-request API is not the surface to check, and a `422` reported for it did not reproduce.**
  ...
  **Re-measured 2026-08-06 and still exhausted, now with a wider denominator:** across `Morrison-Lab/ai-config`'s last 60 merged PRs, every Copilot review object carries the refusal body and **zero** are substantive (query in [`shared/workflow/pr-on-claim.md`](../shared/workflow/pr-on-claim.md), which also explains why the object count drifts between runs while the zero does not).
  Every one of those refusals postdates the override rather than explaining why it was written: adding `on_or_after_2026_08_04: ([.[] | select(.submittedAt >= "2026-08-04")] | length)` to that same query returns **39 of 39**, earliest `2026-08-04T04:38:12Z` and latest `2026-08-06T09:14:23Z`, with zero before it.
  So requests are still being issued despite this override, and the override is not merely advisory bookkeeping --- each violation spends quota on a guaranteed non-verdict.
