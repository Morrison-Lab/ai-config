...
- [ ] **No reviews in flight:** no review check runs are in progress and no review requests are pending;
  consensus clean verdicts have landed across all active reviewers with none still running (ai-config#3570).
...
1. **A totally clean review consensus on the latest pushed commit** ---
   no nits, no non-blocking comments, no informational notes,
   everything Addressed or agreed Deferred,
   evaluating the exact HEAD SHA currently on the branch across all active reviewers
   with no reviews still running (ai-config#3570).
