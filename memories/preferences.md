...
- All agents --- the top-level session and every dispatched subagent --- should keep a to-do/task checklist (`TaskCreate`/`TaskUpdate`/`TaskList` when available) covering not just direct work items but also entries for managing subagents' work and for checking in on long-running background processes.
  ...
  **"When available" is load-bearing, not a hedge: as of Claude Code v2.1.233 these tools are off by default on Opus 4.8/Sonnet 5/Fable 5/Mythos 5 and newer** (`CLAUDE_CODE_ENABLE_TODO_TOOLS=1` restores them) --- check the session's actual tool list before relying on this, and fall back to CLAUDE.md's on-disk lab notebook otherwise.
  See `memories/claude-code.md`'s "`TaskCreate`/`TaskGet`/`TaskUpdate`/`TaskList`/`TodoWrite` are off by default" section.
