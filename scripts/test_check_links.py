#!/usr/bin/env python3
"""Tests for scripts/check-links.py link and autolink validation."""
...
class TestAutolinkRegex(unittest.TestCase):
    def test_uri_autolinks_matched(self):
        matches = [
            m.group(1)
            for m in check_links.AUTOLINK.finditer(
                "<https://example.com> and <http://foo.bar/baz>"
            )
        ]
    ...
    def test_email_autolinks_matched(self):
        matches = [
            m.group(1)
            for m in check_links.AUTOLINK.finditer(
                "<alice@example.com> and <bob.smith+tag@sub.domain.org>"
            )
        ]
...
class TestMarkdownTableLinks(unittest.TestCase):
    def setUp(self):
        check_links.broken = []
        check_links.checked = 0

    def test_table_with_autolinks_and_pipes(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            td = Path(tmpdir)
            guide = td / "guide.md"
            guide.write_text("# Guide", encoding="utf-8")
            table_file = td / "table.md"
            table_file.write_text(
                "| Name | Link | Contact | Doc |\n"
                "| --- | --- | --- | --- |\n"
                "| Alpha | <https://example.com/alpha> | <alpha@example.com> | [Guide](guide.md) |\n"
                "| Beta | <mailto:beta@example.org> | <beta.user+team@domain.co.uk> | [Guide](<guide.md>) |\n"
                "| Gamma | <http://example.com/q?a=1|b=2> | <user@domain.tld> | [Guide](guide.md#sec) |\n",
                encoding="utf-8",
            )
            check_links.check_file(table_file, root=td)
            self.assertEqual(check_links.broken, [])
            self.assertEqual(check_links.checked, 3)

    def test_table_with_adjacent_pipes_and_fences(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            td = Path(tmpdir)
            table_file = td / "table.md"
            table_file.write_text(
                "|Col 1|Col 2|Col 3|\n"
                "|---|---|---|\n"
                "|<https://example.com>|<user@domain.tld>|`<https://ignored.com>`|\n"
                "|`<invalid@broken>`|`|`|<mailto:support@domain.tld>|\n",
                encoding="utf-8",
            )
            check_links.check_file(table_file, root=td)
            self.assertEqual(check_links.broken, [])
    ...
    def test_table_with_broken_relative_links(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            td = Path(tmpdir)
            table_file = td / "table.md"
            table_file.write_text(
                "| Col 1 | Col 2 |\n"
                "| --- | --- |\n"
                "| [Missing](missing_file.md) | <https://example.com> |\n"
                "| <user@domain.com> | [Missing 2](<missing_dir/file.md>) |\n",
                encoding="utf-8",
            )
            check_links.check_file(table_file, root=td)
            self.assertEqual(
                check_links.broken,
                [
                    "table.md -> missing_file.md",
                    "table.md -> missing_dir/file.md",
                ],
            )
