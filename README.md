...
| `no-push-without-self-review.py` | `PreToolUse` (Bash) | blocks `git push` unless a separate `adversarial-reviewer` subagent returned a clean verdict as its own call result, with no file edit recorded since, or `ALLOW_UNREVIEWED_PUSH=1` is provided; a verdict quoted anywhere else in the transcript (including this guard's own denial) does not count |
