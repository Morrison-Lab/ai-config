...
| `warn-pr-create-without-dupe-check.py` | `PreToolUse` (Bash) | warns when a command creates a PR and no earlier command in the session could have surfaced an already-open one; warns rather than blocks, since a duplicate PR is cheap to close and a blocked creation is not |
