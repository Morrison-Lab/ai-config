...
| `MERGE_PR` | Merge a pull request, pinned to the head the clean gate evaluated; on a base that requires a merge queue only the CLI enqueues, since the MCP tool calls the direct merge endpoint, so an MCP-only session stops there. | `gh pr merge "<N>" -R "<owner>/<repo>" --match-head-commit "<sha>"` | `mcp__github__merge_pull_request (expectedHeadSha="<sha>")` |
