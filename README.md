...
| `warn-unmeasured-capability-claim.py` | `PreToolUse` (Bash, mcp__github__.*) | warns, never blocks, when a forge write about to post -- a `gh`/`glab` issue/PR comment or create/edit, or an MCP issue/PR write -- carries an ABSOLUTE claim that a capability is absent (`is unavailable`, `unbuildable`, `impossible`, `regardless of`, `there is no way to`, `no X can/will Y`) within 400 characters of a tooling noun (harness, guard, hook, dispatch, transcript, agent, classifier, CI...). Both factors are required, which is what keeps it quiet: an absolute idiom with no tooling noun is ordinary prose, and a tooling noun with no absolute idiom is a status comment. A negative-capability claim names no artifact, so unlike a positive claim it cannot be checked by opening the thing it is about, and so gets inferred from an adjacent observation and published as established. Warns rather than blocks because an exhaustively measured claim is lexically identical and is the valuable one; chat replies are out of scope because the cost guarded against is durability, not wrongness (ai-config#3707, #3739; measured 2026-09-17: two such claims posted within an hour, both retracted in public) |
