...
| `flag-nonconvergent-review.py` | `Stop` | warns, never blocks, when four or more adversarial review rounds have run, the latest is not clean, and either a finding category has recurred across three rounds or the last three counts show no downward trend -- asks whether the DESIGN is wrong before another round. Non-convergence is a property of the sequence, invisible to any single round (measured 2026-09-04/05) |
