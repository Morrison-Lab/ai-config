...
- **Not destructive or irreversible actions.**
  ...
  **Local git housekeeping is the exception, and it is inside the grant.**
  ...
  - **Deleting a local branch** needs its content already on the default branch.
    ...
    [`pr-on-claim`](../../shared/workflow/pr-on-claim.md)'s "One reading it does not cover" section carries three measured cases: two where the two-dot diff ran to thousands of lines purely because the default branch had since taken later, unrelated merges, and a third where the path-scoped diff came back empty and correctly confirmed the merge.
