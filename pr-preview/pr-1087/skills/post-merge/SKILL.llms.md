...
The merged PR is **no longer a place to address anything**. That is a third reason for moving work elsewhere, distinct from the two the corpus already carries: ARD’s Defer moves a finding because fixing it would widen the PR’s scope, and [`address-every-comment`](../../shared/workflow/address-every-comment.md)’s `main`-sync case moves one because the line is not yours. Here the finding is in scope and is yours, and only the branch it belonged to is gone.
