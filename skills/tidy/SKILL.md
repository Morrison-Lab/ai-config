...
- **`code-editor` agent** ([`.claude/agents/code-editor.md`](../../.claude/agents/code-editor.md)) --- the dedicated code editor subagent.
  Coding workflows work iteratively: implement functionally-correct code first,
  then hand off to `code-editor` to enforce style guide compliance and refactor cleanly.
  While `/tidy` audits complexity and produces prioritized recommendations,
  `code-editor` can be dispatched to implement approved simplification and cleanup.
