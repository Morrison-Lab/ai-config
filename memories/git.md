...
## `git gc --prune=now` on a full disk corrupts pack files

Running `git gc --prune=now` (or any aggressive repack) when the disk is at or
near 0 bytes free corrupts the resulting pack file: objects are partially
written, deltas left unresolved, and `git fetch` / `git pull` thereafter fails
with `fatal: unresolved deltas left after unpacking` or
`error: Could not read <sha>`.

The corruption is unrecoverable locally: deleting loose objects and pack files
does not help because the new pack downloaded during `fetch` still tries to
resolve deltas against the (missing) corrupted objects, and fails.

The only reliable recovery is a full re-clone:
```sh
rm -rf .git
git clone <remote-url> <dir>
```

Prevention: never run `git gc` when disk space is low.
The "low" threshold is effectively "any time you have recently seen
`No space left on device`" --- the tool does not check free space before
beginning the repack, and a partial write is worse than no gc at all.

- **Do:** check `df -h` or equivalent before `git gc`, and skip it when space
  is tight.
- **Do:** re-clone immediately when `git fetch` starts failing with unresolved
  deltas after a gc.
- **Don't:** run `git gc --prune=now` as a troubleshooting step for disk space
  --- it consumes space during the repack and can corrupt the repo if the
  disk fills mid-write.
- **Don't:** try to repair a corrupted pack by deleting individual objects ---
  the pack references them and the corruption cascades.

(Measured 2026-08-20: `git gc --prune=now` ran at 0 bytes free on
Morrison-Lab/ai-config, corrupted 13+ objects across multiple pack files,
required full re-clone to recover.)
