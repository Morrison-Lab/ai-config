...
| [`git-worktrees.md`](git-worktrees.md) | Git worktrees | Worktree-specific behavior: `worktree add`/`remove` gotchas, submodules inside a worktree, `checkout -B` bypassing the already-checked-out guard, pushing by refspec when a branch is checked out elsewhere, and two worktrees sharing one branch ref. Split out of `git.md` (ai-config#694 pattern) at the 1200-line gate. |
