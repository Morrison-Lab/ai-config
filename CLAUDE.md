...
- **`git commit -m` is the surface that enumeration hides**, because every other entry posts to GitHub, so a commit message reads as a different kind of thing while the shell treats it identically.
  ...
  - **Do:** put that file in the session scratchpad, not in the worktree and not at a bare `/tmp` path --- `git add -A` stages one written inside the worktree and the commit ships it, while `/tmp` is shared with every concurrent session (`memories/shell.md`).
