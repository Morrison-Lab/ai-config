...
The source lives at `skills/convert-repo-format/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/convert-repo-format`, resolve the symlink target for this wrapper directory first, then read `../../skills/convert-repo-format/SKILL.md` relative to that real directory.
