...
check("enabled ai-config plugin is detected", enabled('[plugins."ai-config@d-morrison"]\nenabled = true\n'))
check("disabled ai-config plugin is ignored", not enabled('[plugins."ai-config@d-morrison"]\nenabled = false\n'))
...
check("quoted false is not treated as enabled", not enabled("[plugins.'ai-config@d-morrison']\nenabled = false # keep installed but off\n"))
