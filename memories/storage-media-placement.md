...
The `wsl --import`/`--move` arm is the one exception, and it is deliberate: the verb names both the operation and its subject, so a single drive letter and no recognised toolchain path still warn.
Its SOURCE tar is exempt from the backup narrowing too: a `wsl --export` tar idiomatically lands under a backup directory, and testing it there made the canonical export-then-import recipe the silent one.
