...
| `flag-positional-figure-in-commit-message.py` | `PreToolUse` (Bash) | warns, never blocks, when a `git commit` message about to be written states a positional or dimensional figure about text --- "13 lines above", "39 lines below", "77 lines earlier", "~130 lines later", "the 60-to-80 range" --- since a commit message is permanent history and the count is re-derived by nobody: true when typed, false as soon as anything above it changes. The warning says to DELETE the number rather than correct it, naming the target instead of counting to it. Measured on this repository's own history (2026-09-03): 14 of 2412 commits carry the shape and every one is decoration, while a bare `N characters`/`N chars` with no positional word matches 53 legitimate measured facts and is deliberately not matched |
