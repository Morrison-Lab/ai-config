...
| `no-misattributed-quote.py` | `Stop` | **not registered ([#1505](https://github.com/Morrison-Lab/ai-config/issues/1505))** --- would block a reply attributing a quoted phrase to a corpus file that does not contain it, when that phrase is in the file's `.rationale.md`/`.cases.md` sibling; stays silent when the phrase is found nowhere else, since a bare "not found" is the invented-quote misread |
