...
- Before the merge command, run the base-currency check that `fully-clean.md` states in its stale-base rule (the Do bullets beginning "for a direct merge"), by merge mode:
  ...
  in a remote session without `git`, read `GET repos/<owner>/<repo>/compare/<base>...<head-sha>` and require `behind_by` of 0, and where neither is available do not merge until [#2982](https://github.com/Morrison-Lab/ai-config/issues/2982) supplies the tool;
