...
3.  When a PR reaches 100% clean state, merge it immediately (default: squash merge via `gh pr merge "<number>" -R "<owner>/<repo>" --squash --delete-branch --match-head-commit "<pinned-sha>"`, the pin being the `headRefOid` recorded before the instrument ran), verify the merge landed on GitHub/GitLab, and run the post-merge skill (`post-merge` / `ums`).
