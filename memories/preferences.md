...
- Always leave yourself handoff notes proactively when pausing --- don't wait to be asked --- especially while a long-running job is in flight (SLURM arrays, builds, CI, background tasks, remote agents).
  Snapshot branch/HEAD, unpushed commits, job IDs + how to check status, expected outputs + paths, backups, open decisions, and the exact pick-up steps, post that snapshot on the forge --- a comment on the active PR/MR, a comment on the issue, a new issue when there is neither, or a committed file for state that outlives the thread --- and keep a project memory as the backup.
