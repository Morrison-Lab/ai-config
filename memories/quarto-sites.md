...
Hit while adding a mobile within-chapter TOC to `the repository owner/rme` (#929); apply to
...
- **Manuscript projects do NOT support `repo-url` / `repo-actions` natively.**
  ...
  via inline JS — see `the repository owner/qmt/_repo-links.lua` for a full implementation.
...
Verified from a clean state (`rm -rf _site .quarto` first,
  no priming single-file renders) on `the repository owner/macros`: `_quarto.yml` there
...
Put the path under front-matter or
  ...
  `shortcodes: [../_extensions/the repository owner/slidebreak/slidebreak.lua]`), even
...
(Measured 2026-08-24 on `the repository owner/macros`, whose `_quarto.yml` declares `format: html` only while `demo-shortcode.qmd` and `demo-include-in-header.qmd` each declare their own `pdf:` and `revealjs:` blocks --- demonstrating the macros reaching the LaTeX preamble being the entire point of those two pages.
Caught in self-review on [macros#83](https://github.com/the repository owner/macros/pull/83) before the TinyTeX removal merged.)
