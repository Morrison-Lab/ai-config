...
| `no-fable-subagent.py` | `PreToolUse` (Agent, Task) | denies an Agent dispatch that names Fable without `ALLOW_FABLE_SUBAGENT=1`, or that names no `model` while the transcript shows the session itself is on Fable (the worker would inherit it); an omitted `model` on any other or unknown session only warns; every other explicit model passes through (ai-config#2929) |
