...
- The renv library is NOT provisioned at session start (`requireNamespace`
  ...
  - the repository owner GitHub-only pkgs → r-universe `https://the repository owner.r-universe.dev`
...
- **A fresh `git worktree` gets its own renv library cache, keyed by the
  ...
  surfaced. (`the repository owner/rme#1009`, several rounds: a background
...
- **A `renv::restore()` failure downloading `https://api.github.com/repos/<owner>/<repo>/...`
  ...
  points. (`the repository owner/rme#772`, tracked in `the repository owner/rme#994` and `the repository owner/rme#996`, fixed centrally in
  `the repository owner/gha#241`.)
...
- **renv activation failure when a GitHub remote is blocked**: if `DESCRIPTION`
  ...
  `the repository owner/altdoc@recursive-qmd-search`: a plain `curl` to
  `api.github.com/repos/the repository owner/altdoc/...` 403'd with `"GitHub access to
...
- **When a consumer repo pins a dependency to an upstream *feature branch*
  ...
  *in* that dependency, the fix must land ON that pinned branch --- open a PR
  ...
  `the repository owner/altdoc@recursive-qmd-search`; fixing the consumer's docs build
...
- **`snapr` is not on CRAN or P3M**: install from the GitHub tarball.
  `curl -L https://codeload.github.com/the repository owner/snapr/tar.gz/refs/heads/main -o /tmp/snapr.tar.gz`
...
- The `latex-macros` submodule (the repository owner/macros) is uninitialized on a fresh
...
- **The same deletion fires from a second, likelier trigger: two full suites
  ...
  *different* tests.
  ...
  one clean run, alone.
  (`the repository owner/altdoc#61`, 2026-07-25: two overlapping `test_local()` runs;
