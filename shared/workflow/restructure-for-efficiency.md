...
- **Work sitting at the conductor's tier that a cheaper or unbilled one could do.**
  [`delegate-to-opencode`](../../skills/delegate-to-opencode/SKILL.md) offers an unbilled hosted Zen tier and a subscription Go tier, while [`delegate-to-codex`](../../skills/delegate-to-codex/SKILL.md) is separately billed, so both come ahead of this session's own quota for bounded mechanical work.
  Never use a local or on-device model as the cheaper route.
