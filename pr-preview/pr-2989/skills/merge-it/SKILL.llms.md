...
- Then record that PR’s `headRefOid` and `baseRefName` before the readiness check below, so its result is tied to one head and one target, and require both live values to equal them immediately before every direct merge, on the initially current path as much as after a recovery.
