...
The source lives at `skills/recover-followups/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/recover-followups`, resolve the symlink target for this wrapper directory first, then read `../../skills/recover-followups/SKILL.md` relative to that real directory.
