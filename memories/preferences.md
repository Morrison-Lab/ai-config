...
- **Mark review-ready PRs ready before ending a delivery turn**, even when the harness opened them as drafts.
  Cloud `ManagePullRequest` / `create_pr` defaults
  and the up-front empty-PR pattern both leave PRs in draft;
  AGENTS.md overrides those defaults
  once implementation is on the branch head and checks pass.
