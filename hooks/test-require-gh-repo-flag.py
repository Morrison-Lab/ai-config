...
BLOCK = [
    ...
    ("let x=1<<EOF\n" + G + "pr merge 456 --squash\nEOF\necho done",
     "let arithmetic shift does not mask ungated command"),
    ("let \"x=1;2<<EOF\"\n" + G + "pr merge 456 --squash\nEOF\necho done",
     "quoted let arithmetic shift does not mask ungated command"),
    ("declare -i x=1<<EOF\n" + G + "pr merge 456 --squash\nEOF\necho done",
     "declare -i arithmetic shift does not mask ungated command"),
    ("declare -i x=\"1;2<<EOF\"\n" + G + "pr merge 456 --squash\nEOF\necho done",
     "quoted declare -i arithmetic shift does not mask ungated command"),
    ("x=$[1<<EOF]\n" + G + "pr merge 456 --squash\nEOF\necho done",
     "$[ expr ] arithmetic shift does not mask ungated command"),
    ("x=(( a << 1 ))\n" + G + "secret set FOO",
