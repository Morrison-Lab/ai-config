...
Whether ai-config has an automatic review to race against is a separate fact, and it changed.
[#1707](https://github.com/Morrison-Lab/ai-config/pull/1707) restored the `pull_request` trigger on 2026-08-20, so a dispatch now CAN race a push-triggered run here.
Measured on [#1724](https://github.com/Morrison-Lab/ai-config/pull/1724) that day: run `32345965633` (`pull_request`) was cancelled by run `32345990687` (`workflow_dispatch`), leaving `review / require-review` red.
