...
- Keep PRs focused on a single concern: never mix CI/workflow infrastructure changes (`.github/workflows/`) with heavy simulation/validation dataset artifacts (`inst/extdata/*.rds`) or HPC job array updates in the same PR --- open dedicated PRs per concern (see `split-concerns`).
