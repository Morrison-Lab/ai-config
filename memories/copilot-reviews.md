...
- **An empty BALANCED "Needs a closer look" review is non-blocking.
  The identical review at LITE effort still blocks.**
  ...
  When it carries `**Review effort:** Balanced`, `**Findings:** None`, no "Open (N)" details block, and no "Previously missed" item, there is no finding behind it at all --- `scripts/check-pr-fully-clean.py`'s `copilot_verdict()` used to treat ANY `Needs a closer look` occurrence as `not-clean` unconditionally, so this exact shape blocked a merge with nothing to address ([Lacaedemon/sparta#1638](https://github.com/Lacaedemon/sparta/pull/1638), [review 5316721676](https://github.com/Lacaedemon/sparta/pull/1638#pullrequestreview-5316721676), `b36fe3bb`, 2026-09-25T10:42:37Z).
  Fixed in [ai-config#4004](https://github.com/Morrison-Lab/ai-config/issues/4004): the carve-out reads the body as NO VERDICT (`""`), never as `clean` --- it must not count toward a clean-review quorum, and it must not supersede an earlier not-clean Copilot verdict from an earlier round.
  `Changes recommended` stays unconditionally blocking regardless of effort or Findings count.
  Only `Needs a closer look` gets the carve-out.
  ...
  `copilot_is_empty_balanced_closer_look()` there went through THREE successive heuristic designs, and a fresh adversarial probe defeated each one in turn.
  The first read the heading/effort/findings patterns anywhere in the raw body, and was defeated by the shape stated only inside an HTML comment or a collapsed `<details>` section.
  The second required those patterns to be LIVE (comment/`<details>`-stripped text), and was defeated by a live heading BEFORE the v2 overview marker combining with an unrelated, heading-less Balanced/None block AFTER it, since nothing required the two to belong to the same overview.
  The third scoped the live check to the block after one confirmed-live marker, and was defeated by a bare marker with no `## Copilot review overview` line at all, by the marker (or the effort/findings lines) sitting inside a fenced code block or an inline code span, and by CRLF line endings.
  Each fix closed exactly the probe that found it and left the next one open, because a heuristic enumerates ways a shape can be EXCLUDED and a reviewer only has to find one exclusion it missed.
  Fixed (so far) by inverting the approach entirely: `copilot_is_empty_balanced_closer_look()` now normalizes line endings and strips trailing whitespace per line, then runs ONE `re.fullmatch` of the WHOLE body against a strict template.
  That template is: the exact marker line, blank lines, the exact `## Copilot review overview` line, blank lines, a `###` heading line for `Needs a closer look` (an emoji or other non-letter prefix allowed, nothing else), blank lines, EXACTLY ONE prose paragraph, blank lines, the exact `**Review effort:** Balanced` line immediately followed by the exact `**Findings:** None` line, and optional trailing blank lines, with NOTHING else anywhere.
  A template enumerates the single shape the carve-out APPLIES to instead of the ways a body can be excluded from it, so it fails closed on any character this function does not explicitly expect -- a details block, a code fence, an extra section, a second marker -- rather than only on the specific deviations a probe has already tried.
  This is STRICTER than `check-pr-fully-clean.py`'s own carve-out on purpose: a real empty-Balanced review that also lists resolved items in a trailing `<details>` (a legitimate, common shape once a PR has had a prior round) stays NOT_CLEAN in this gate even though the more sophisticated checker would correctly read it as no verdict, which is the safe direction for a second, independent, coarser classifier to err in.
  The template alone was STILL not the whole fix: its prose-paragraph rule only forbade a line's OWN FIRST character from being `#`, `*`, `<`, a backtick, or `~`, so plain prose reading "Previously missed items were checked and none remain outstanding." or "See the Open (1) section" -- nothing forbidden at the START of the line -- matched the template outright, and a `<strong>Open (1)</strong>` sitting MID-SENTENCE inside an otherwise ordinary-looking prose line did too, since only the line's first character was ever checked.
  Fixed two ways at once: the prose-line pattern now forbids a `<` ANYWHERE in the line, not just as its first character (and, as shipped, a `*` anywhere too -- ``_COPILOT_TEMPLATE_PROSE_LINE = r"(?![ \t]*[#`~])[^\n<*]+"``, `plugins/ai-config/enforce-mwc-review-gate.py:464`, so a stray `**field:**`-looking fragment mid-sentence cannot hide as prose either), and two raw-body guards -- a case-insensitive `previously missed`, and `Open (` followed by digits and `)` with whitespace tolerated inside the parens (`COPILOT_OPEN_COUNT = re.compile(r"Open[ \t]*\([ \t]*[0-9]+[ \t]*\)", re.IGNORECASE)`, same file:493, e.g. `Open ( 1 )`) -- are checked ALONGSIDE the fullmatch and fail the carve-out closed regardless of where in the body either phrase appears, mirroring `check-pr-fully-clean.py`'s own `COPILOT_PREVIOUSLY_MISSED`/`COPILOT_OPEN_ITEMS_HEADING` guards but broader (no `<strong>` wrapping required, since the bare phrase in plain prose is just as real a finding).
  Four more gaps surfaced in [ai-config#4008](https://github.com/Morrison-Lab/ai-config/pull/4008)'s own review rounds (a Claude re-review plus Copilot's own automated review of the #4004 PR, reviewing the carve-out that reviews Copilot):
  1. `scripts/lib/copilot_overview.py`'s `copilot_v2_block_review_efforts()` did not exclude an HTML-comment-hidden `**Review effort:**` line the way its sibling `_copilot_v2_findings_count` excludes a comment-hidden `**Findings:**` line, so a body with NO live effort statement anywhere -- only one hidden inside a comment -- still read as consistently Balanced;
     it also silently dropped a second, EMPTY live effort line via `if rest`, instead of keeping it as an invalid value that breaks the "every live line reads Balanced" equality check.
     Both are now handled: comment-hidden matches are excluded via `_position_in_spans` against `_find_html_comment_spans`, and every live value (including an empty one) is kept in the returned list.
  2. `scripts/check-pr-fully-clean.py`'s `_copilot_is_empty_balanced_closer_look()` returned before ever reaching a `COPILOT_SUPPRESSED_BLOCK` veto of its own, so a Balanced closer-look body with `Findings: None` and a genuine `Suppressed comments` block carrying a real finding (one naming neither "Previously missed" nor an "Open (N)" listing) read as no verdict instead of not-clean.
     `COPILOT_SUPPRESSED_BLOCK.search(scan)` is now checked alongside the other two body-wide guards.
  3. The Antigravity gate's own `_COPILOT_TEMPLATE_PROSE_LINE` lookahead only inspected a line's own FIRST character, so a fence or heading indented by leading spaces (e.g. three spaces then a code fence, or three spaces then `###...`) still matched as ordinary prose -- the lookahead now rejects any run of leading spaces/tabs immediately before `#`, a backtick, or `~`, not just the character in the line's own first position.
  4. `latest_bot_review_states()`'s carve-out relied only on the overview body's own `**Findings:** None` line, never checking whether a separate inline review comment/thread from that same bot was still live on the current head -- `fetch_pr_data()` now also fetches the PR's inline review comments (`pulls/{n}/comments`, a different REST endpoint from the issue-comments fetch), and the carve-out is gated on finding none tied to the current head (`commit_id`/`original_commit_id` matching `headRefOid`) from the same bot login.
  Tested in `scripts/test_enforce_mwc_review_gate.py` against the real, verbatim body of [Lacaedemon/sparta#1638](https://github.com/Lacaedemon/sparta/pull/1638)'s [review 5316721676](https://github.com/Lacaedemon/sparta/pull/1638#pullrequestreview-5316721676) (and the same body under CRLF), which the template must always match, alongside every adversarial probe that defeated an earlier design, a harmless trailing "Resolved since last review" block that the template still refuses, and -- separately from that trailing-content case -- `Previously missed`/`Open (N)` stated INSIDE the one permitted prose paragraph, with and without `<strong>` markup.
  - **Do:** read an empty `Needs a closer look` review at Balanced effort (no Open/Previously-missed anywhere in the body, Findings: None) as no verdict, and keep waiting on or re-requesting review rather than treating it as a blocker to ARD.
  - **Do:** keep treating the identical shape at Lite effort, or one carrying an "Open (N)" listing or a "Previously missed" item -- whether in a trailing block or stated in plain prose -- as a real not-clean verdict, in every classifier in this repo.
  ...
  - **Do:** when a second, independent classifier re-derives a structural check that another tool already gets right through careful block-and-citation parsing, reach for a strict whole-body template over a growing sequence of heuristic patches -- a template's failure mode (denying a shape it should have allowed) is the safe direction, and a heuristic's failure mode (allowing a shape it should have denied) is not.
  - **Do:** check a structural template's own character-class exclusions for whether they apply to a whole line or only to its first character -- a first-character-only check reads as complete while leaving every later character in the line unchecked, which is exactly where a real finding's own vocabulary can hide.
  ...
  - **Don't:** treat a third or fourth heuristic refinement of the same structural check as progress once two in a row have already been defeated by fresh probes -- that recurrence is the signal to change the approach's SHAPE, not to patch it again.
  (Directive from the user, 2026-09-25: "you can ignore empty 'balanced' copilot reviews like [Lacaedemon/sparta#1638#pullrequestreview-5316721676](https://github.com/Lacaedemon/sparta/pull/1638#pullrequestreview-5316721676)".
  Tracked as [ai-config#4004](https://github.com/Morrison-Lab/ai-config/issues/4004).)
