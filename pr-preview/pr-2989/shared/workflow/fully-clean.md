...
- **Do:** for a direct merge from a remote session without `git`, read the compare endpoint instead, `gh api "repos/<owner>/<repo>/compare/<base>...<head-sha>"`, and require `behind_by` of 0.
...
- **Do:** when the merge-base is not that tip and the merge is direct, update the branch pinned to the recorded head (the `expected_head_sha` call below, or `update_pull_request_branch` with `expectedHeadSha` remotely), then rerun the whole clean gate on the new head, review included, before merging.
  ...
  A `422` on that call means the head already moved, which is the another-writer signal below, so it routes to the ownership rule instead of merging the base into someone else's push.
  Measured 2026-09-02 (Pacific) on [#2989](https://github.com/Morrison-Lab/ai-config/pull/2989): a deliberately wrong `expected_head_sha` returned `422` with "expected head sha didn't match current head ref" and changed nothing.
