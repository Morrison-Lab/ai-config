...
The source lives at `skills/split-concerns/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/split-concerns`, resolve the symlink target for this wrapper directory first, then read `../../skills/split-concerns/SKILL.md` relative to that real directory.
