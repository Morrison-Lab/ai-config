...
Avoiding a write-capable shell command is instruction-level discipline rather than a
harness guarantee, so it is on you: do not run anything that writes, moves, or deletes
a file, pushes, or posts.
