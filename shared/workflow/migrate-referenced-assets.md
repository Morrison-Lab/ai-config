...
content into the build dir (fixed upstream in the repository owner/altdoc's
...
migrated the macros to the `the repository owner/macros` submodule while the pkgdown-to-altdoc
PR was open; pkgdown had rendered fine, altdoc's staged build could not find
`../macros/macros.qmd`.)

(the repository owner/wai#6, 2026-07-14: a chapter migration from one Quarto repo to
