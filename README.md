...
| `flag-unmeasured-timestamp.py` | `PreToolUse` (Bash, mcp__github__.*) | warns, never blocks, when a `gh pr comment`/`gh issue comment`/`gh pr review`/`gh api .../comments` body, or the `body` of one of the `mcp__github__` comment tools that `require-agent-disclosure.py` covers, about to post states a Pacific clock time (the Stop sibling's own stamp shape: `HH:MM`, optional seconds, optional AM/PM, then `PDT`, `PST`, or `PT`) and no clock read appears in the transcript since the current turn began, or the stamp runs ahead of the harness's injected reading -- the forge-comment half of the timestamp rule, whose chat-recap half is `no-unmeasured-clock-claim.py` at `Stop`, and whose past-action, UTC-conversion, and scheduled-check-in exemptions it imports; the warning names the stamp and the `TZ=America/Los_Angeles date` command to run before restating it, and a body the check cannot read (a `--body-file` not yet on disk, `--body-file -`) gets a cannot-read note rather than silence, after `require-agent-disclosure.py`'s third verdict (ai-config#2900's claim comments stamped up to an hour ahead of the clock, #2903) |
