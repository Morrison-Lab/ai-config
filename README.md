...
| `no-fable-subagent.py` | `PreToolUse` (Agent, Task) | denies an Agent dispatch that names no `model` (it would inherit the session's, so on a Fable session a Fable worker) or names Fable without `ALLOW_FABLE_SUBAGENT=1`; every other explicit model passes through (ai-config#2929) |
