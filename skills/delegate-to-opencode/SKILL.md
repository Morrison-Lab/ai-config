...
- **The critical-path edit the rest of the work waits on.**
  Keep it local so progress does not block on a round-trip.
- **The result must conform to a schema and you have no cheap validator.**
  `opencode run` has no schema flag (see step 2), so conformance is asked for in the prompt and checked on the way back rather than enforced at the boundary.
- **The hosted tier is rate-limited, or the ollama daemon is down.**
  ...
  This is availability, not budget, and what it licenses depends on the routing rule below.
...
**The split.**
`ollama/*` runs against the base URL the config declares for the ollama provider, `http://localhost:11434/v1` as of 2026-08-19.
The payload does not leave the machine.
No other destination on the ladder --- not Claude, not codex, not agy, not opencode Zen --- has that property.
...
The prefix answers the routing one.
...
`memories/preferences.md`'s "Delegate heavy work to a separately-billed CLI first" holds the order across `codex`, `agy`, and `opencode`.
...
`opencode run` takes the message as positional arguments, and `-f/--file` attaches files.
...
`opencode run --help` lists none, and permissions come from the `permission` block in the opencode config.
...
For a long or multi-item run, use the background-runner-plus-DONE-marker pattern in [`delegate-to-codex`](../delegate-to-codex/SKILL.md) step 3 rather than a foreground call that the tool timeout will kill.
One thing there does not transfer: **that runner's `MAXPAR` fan-out applies to the hosted tier and not to the local one.**
