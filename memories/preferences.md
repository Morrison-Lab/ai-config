...
- Before pushing skill/memory changes to ai-config, run the local checks that `validate.yml` runs in CI --- at minimum `python3 scripts/validate-skills.py` and `python3 scripts/check-links.py`, for frontmatter and broken-relative-link errors --- before they cost an ARDI round.
  Derive the full list from `.github/workflows/validate.yml` rather than from this bullet, which named only those two while the job had grown well past them --- `python3 scripts/run-local-validation.py` is that derivation (ai-config#1940; `--list` shows the plan, `--only REGEX` runs a slice).
