...
*(When delegating UMS to a subagent, prefer **session forking** so the subagent can survey the full conversation history directly without manual serialization from the parent orchestrator — see [`use-subagents`](../../shared/workflow/use-subagents.md).)*
