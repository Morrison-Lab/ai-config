...
- **Example (3rd occurrence, mechanism unconfirmed)**: 2026-09-14, proposed on Morrison-Lab/ai-config#3657: a bare `git push -u origin <literal-branch-name>` (no pipe, no `$VAR`, no `-C`) was refused three times in a row, twice citing the literal branch name as unresolvable and once citing a genuinely clean verdict as "blocking" despite zero findings.
  `git rev-parse <branch>` succeeded in the same shell throughout.
  Unlike the first two examples, no suspicious token is visible in the refused command text, so this may be a third, distinct trigger -- possibly the shared git-call time budget (`_run_git`'s own docstring) being exhausted by parsing a large transcript (a 163k-token adversarial-reviewer dispatch) before reaching the resolution call.
  Not independently confirmed; recorded as a candidate rather than a diagnosed cause.
  `ALLOW_UNREVIEWED_PUSH=1` was used after a genuine foreground review had already returned clean against the exact pushed commit, disclosed in the PR body.
