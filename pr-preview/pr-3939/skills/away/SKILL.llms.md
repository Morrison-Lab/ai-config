...
Distinguish from [`daytb`](../../skills/daytb/SKILL.llms.md), or a one-off “just proceed” on a single pending question — `daytb` covers one decision and its same-task follow-ons, expiring with that task; `away` is a session-scoped mode for everything still to come.
