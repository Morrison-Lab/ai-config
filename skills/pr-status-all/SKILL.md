...
>    Match inside the collapsed `<details>` region, case-insensitively on `suppressed` -- not on either exact phrase, not on `<summary>` alone, and not anywhere in the body.
