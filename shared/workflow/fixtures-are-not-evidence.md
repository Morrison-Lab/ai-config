...
A driver-ledger classifier's fixtures modelled the two kinds of comment it needed to tell apart, but omitted the agent-disclosure marker (`_Posted by Cursor Grok 4.6 (AI agent) --- not written by a human._`, since `cursor[bot]` posted both) that BOTH real captured driver comments the classifier was built from (GitHub comment ids 5430672892 and 5430978306 on ai-config#2341) actually carry.
...
could not see it.
The PR's own review history records that shape recurring three times before
this finding, on different call sites each time, twice inside a fix written
in direct response to the previous instance.)
