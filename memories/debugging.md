...
Hit on the repository owner/ai-config#265: the semantic-line-breaks script lacked fence
...
- `actions_list` responses for an active repo can exceed the tool's token cap
  ...
  line. (`the repository owner/altdoc#16`: confirmed `jarl-check`'s failures via
...
Lessons the reviewer flagged across the `session-lock` PR (the repository owner/ai-config#38) —
...
Hit on the repository owner/rme#966/#967: the Quarto **publish** workflow (push-to-main
...
that asserts the invariant is enough. the repository owner/rme#970 added `check-render-headers`, a
...
- Fix: dispatch the repo's `cleanup-pr-previews` workflow (the
  `the repository owner/gha` reusable: deletes previews for non-open PRs, then
...
- This is strictly better than the "403 on the docs page, so raw-fetch its
  `.qmd` source instead" fallback in `the repository owner/gha`'s `CLAUDE.md`: the
...
- Fixed generally in the `the repository owner/altdoc` fork (altdoc#25/#26): the
...
(Correction logged from review on the repository owner/ai-config#641, 2026-07-22.)
