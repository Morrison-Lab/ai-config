...
| `flag-test-reading-package-source.py` | `PreToolUse` (Write, Edit, NotebookEdit) | warns, never blocks, when an edit to a code file under a test directory adds a read of a path escaping that directory -- a read reaching two levels up (directly or via `test_path`/`file.path`), a read on a path beginning `R/`/`src/`/`inst/` (including `here::here("R", ...)`), or a path-then-read chain such as `Path(...).joinpath("src", "m.py").read_text()`. Path constructors alone do not fire; lines are continuation-joined so a `styler`-wrapped call is still seen. Such a test passes in a source checkout and fails wherever the code is consumed as an installed artifact, since an installed R package ships no `R/*.R`; suggests reading the loaded artifact (`deparse(body(f))` over `asNamespace()`, `inspect.getsource`) instead (ucdavis/hac.sap#43) |
