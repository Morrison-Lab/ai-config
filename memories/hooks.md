...
| [`no-fable-subagent.py`](../hooks/no-fable-subagent.py) | `Agent`, `Workflow` | Block | Denies an Agent launch on Fable, explicit or inherited from a Fable session, without the user's grant; warns on a Workflow launch in a Fable session. | Pass `model: sonnet` (or `haiku`) on the call, or, once the user has approved that specific launch, run it with `FABLE_SUBAGENT_OK=1` for that one command. | `FABLE_SUBAGENT_OK=1` for the one approved launch; never exported for a session. |
