...
| `warn-status-read-after-pipe.py` | `PreToolUse` (Bash) | warns, never blocks, when an expandable `$?` sits in the segment immediately after a pipeline carrying no `pipefail`, so the status read belongs to the trailing formatter rather than to the command. Excludes single-quoted spans and heredoc bodies so the corpus's own documentation of the bug cannot trip it, which also means a `bash -c '...'` one-liner gets no warning |
