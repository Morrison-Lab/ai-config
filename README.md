...
| `no-mutation-in-read-only-reviewer.py` | `PreToolUse` (Bash) | blocks mutating git commands (`commit`, `checkout`, `switch`, `restore`, `stash`, `merge`, `reset`, `rebase`, `revert`, `clean`, branch/tag mutations, `push`) and write tools in read-only personas (`adversarial-reviewer`, `Explore`, `Plan`), preventing shared checkout or stash contamination (Morrison-Lab/ai-config#3612, citing #3602 and #3584); clearable with `ALLOW_READ_ONLY_MUTATION=1` |
