...
sits unread.
   ...
   **If the latest review is a cancellation, the live verdict is stale --- don't re-do already-applied fixes.**
    ...
    **If the reviewer explicitly skips or cannot produce a verdict** (for example, quota exhaustion, an outage, or a policy that prevents a reviewer from self-reviewing its own work), self-review immediately -- don't stall the PR waiting on it.
    In the same round, also check whether a **different** configured external reviewer is available (e.g. Copilot code review, if the repo/org has it). Follow the sequential multi-provider quorum rules in `shared/workflow/adversarial-self-review.md`: request reviews from one provider at a time, and obtain a clean verdict from each before moving to the next. Do not request them in parallel. Self-review is a fallback for when *no* working external reviewer is reachable, never a substitute once one is.
