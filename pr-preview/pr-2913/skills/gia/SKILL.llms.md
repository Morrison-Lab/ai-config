...
**Confirm whose PRs are in scope, too.** Both phases act only on PRs the user opened (`d-morrison` / `dem-extra1`) or is assigned to; `ardia`’s step 1 applies that filter and reports what it dropped. A PR another lab member or a bot opened stays theirs, and an issue such a PR already fixes is left to it rather than grabbed.
