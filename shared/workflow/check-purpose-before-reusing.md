...
Four of `scripts/check-rendered-page.py`'s five detectors read through `_visible_text` (`_list_failed` by way of `_paragraphs`), whose `SKIP_TAGS` excludes `<code>` and `<pre>` subtrees among other things;
`_bad_citation` additionally runs a structural regex over the raw HTML, for a reason its own docstring gives.
