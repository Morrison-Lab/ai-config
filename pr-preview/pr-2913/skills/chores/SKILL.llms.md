...
- Its author passes `memories/reviewing-prs.md`’s scope test for this invocation: a dependency bot (`app/dependabot`, `dependabot[bot]`, `app/renovate`, `renovate[bot]`), the GitHub Actions app (`github-actions`, which opens `chore(submodule):` bumps), or the invoking user, or the invoking user is among its assignees. An explicit `chores` call names the Dependabot/Renovate population, which is what admits those two bots; it admits no other author.
- It looks like a chore: the title starts with `chore(` (e.g. `chore(actions):`, `chore(submodule):`, `chore(deps):`), or the labels include `dependencies`.
...
- ❌ Treating human feature PRs as chores (or vice-versa) — scope by author **and** `chore(` title or `dependencies` label, never by title or label alone.
