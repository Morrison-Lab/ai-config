...
The source lives at `skills/permission-check/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/permission-check`, resolve the symlink target for this wrapper directory first, then read `../../skills/permission-check/SKILL.md` relative to that real directory.
