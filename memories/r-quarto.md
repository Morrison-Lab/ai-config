...
flag requiring revert, not a "cleanup." (`the repository owner/rme#1017`: reverted via `git revert`, then fixed the
...
- `air`'s `air.toml` `[format]` table has a configurable `indent-width`
  ...
  *single* level (one `indent-width`), NOT the styler/tidyverse-style-guide
  ...
  repo that sets `air.toml` `indent-width = 4` (as `the repository owner/altdoc` does)
...
- **`air format . --check` passing is NOT the claim "no line exceeds
  ...
  running air without lintr (the repository owner/altdoc) has no gate at all.
  (the repository owner/altdoc#78, 2026-07-27: two `cli` strings in new code ran to 93
...
- **Fallback when the wanted field isn't supported: `[lint] exclude = ["<dir>/"]`**
  ...
  once the installed jarl version supports it. (`the repository owner/altdoc#18`, #19.)
- **There is no `.jarlignore` file — jarl has never supported one.** Don't
  ...
  (`the repository owner/altdoc#7`: `continue-on-error: true` masked a `.jarlignore`
...
- The **Spellcheck** job (`spelling::spell_check_package()`) fails on any word
  ...
  - **Cross-repo issue refs and bare domain names are spellable-token sources
    ...
    unbackticked `the repository owner/altdoc#26` flags `morrison`, and `rdrr.io` flags
...
- **Codoc mismatch with escaped-dot defaults: limit the lesson to the observed case.**
  In the repository owner/altdoc#30, changing the default regex from an escaped dot (`\\.`) to
  ...
  (the repository owner/altdoc#30, 2026-07-22.)
- **Internal helper functions as default argument values appear literally in `.Rd` usage
  ...
  signature instead. (the repository owner/altdoc#30.)
...
belonging in `the repository owner/gha` since every altdoc repo shares the structure.)
...
(the repository owner/altdoc#64.)
...
(`the repository owner/altdoc` #82/#83/#84, 2026-07-28: this turned "assert the output
...
`rcmdcheck`-driven CI run: `the repository owner/altdoc`'s `R-CMD-check` reports
...
`the repository owner/altdoc#87` predicted the crash for the empty form, and the
opposite is true.
