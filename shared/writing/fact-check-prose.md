...
The second sentence is wrong, and the rule it belongs to says so fifteen lines
earlier --- its trigger is the **second** occurrence.
...
than ordinary care.
The sentence is *true about the neighbour*, so fact-checking its claims passes.
...
- **Don't:** treat "the sentence is true about the other rule" as the check
  having passed --- the question is whether it is true about yours.
...
The construction is widespread here rather than incidental: `grep -rn -i
"pointed the other way\|the mirror of\|inverts cleanly"` over `shared/` returned
9 hits when this was written, and that is one phrasing family among several.)
