...
Moved to [`subagent-worktrees.md`](subagent-worktrees.md) (ai-config#694 pattern, at the 1250-line gate), together with every other section in this file that was about a *second* session's worktree: a subagent's completion report, a shared worktree switched under a live reviewer, an unisolated agent's edits read as your own uncommitted work, a peer worktree arming `no-unshipped-commit.py`, and an empty draft PR.
