...
The source lives at `skills/select-model/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/select-model`, resolve the symlink target for this wrapper directory first, then read `../../skills/select-model/SKILL.md` relative to that real directory.
