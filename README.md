...
| `no-commit-chained-to-push.py` | `PreToolUse` (Bash) | denies a Bash call that chains a `git commit` into a later `git push`. A PreToolUse deny rejects the whole invocation, so a guard refusing the push discards the commit too while its message speaks only about the push (ai-config#2992). Denies rather than warns because every PreToolUse hook runs in one pass: an advisory context cannot reach a failure a sibling's deny has already caused. Clearable with `ALLOW_COMMIT_AND_PUSH=1`; matches over an argv split, so a quoted commit message, a heredoc body, and `git commit-tree` cannot trip it |
