   ...
   **When the user provides a specific review link/ID** (e.g. `#pullrequestreview-4761444085`): Fetch that review directly via the GitHub API using its ID.
   ...
   - **GitHub:**
     ...
     **Match the suppression block on its heading --- a `<summary>` element, or an ATX heading inside a collapsed `<details>` region --- case-insensitively on `suppressed`, not on either exact phrase, not on `<summary>` alone, and not anywhere in the body.**
     ...
     No measured body actually exercises that class, though, so keep the region-wide match as a fallback behind the heading anchor and read a hit only the fallback finds as a prompt to read the region rather than as a finding.
