...
shared [`the repository owner/macros`](https://github.com/the repository owner/macros) submodule
...
```bash
grep -A2 'submodule.*macros' .gitmodules      # confirm path + the repository owner/macros URL
git submodule update --init inst/analyses/macros   # checkout at recorded commit
```

To bring it up to date with `the repository owner/macros`:
...
If a needed concept has no macro, add it to `the repository owner/macros` via a PR to that
repo — do **not** define a one-off command inline in the manuscript:

```bash
cd inst/analyses/macros
git checkout -b add-<concept>-macro
# edit macros.qmd, then push + open a PR to the repository owner/macros
```
