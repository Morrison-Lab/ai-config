...
zero bullets.
An earlier reading of the same query had reported 4 of 5, and the discrepancy is
the point rather than a correction to file away: one merge landed in between, so
a bare count published without its command is stale before anyone reads it.
