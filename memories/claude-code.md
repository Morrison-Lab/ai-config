...
Nothing was actually lost: the run's journal (`subagents/workflows/<run-id>/journal.jsonl`, the same artifact the entry above reads for degenerate synthesis output) records every completed `agent()` result as a `{"type":"result",...}` line, and the file was reconstructed from it afterwards in a few lines of Python. (Measured 2026-08-23, Windows 11 local Claude Code session.)
...
The corruption was caught only by grepping the emitted file. (Measured 2026-08-23, Windows 11 / Git Bash local Claude Code session.)
