...
| `warn-python3-cannot-read-hooks.sh` | `UserPromptSubmit` | names the interpreter when the `python3` on `PATH` cannot read the directory the hooks live in -- on Windows, bare `python3` commonly resolves to the Microsoft Store App Execution Alias, which runs a real interpreter in a packaged-app filesystem view with no access to `%APPDATA%\Claude`. Every hook is registered as `python3 "<plugin root>/hooks/<name>.py"`, and a `PreToolUse` hook that cannot launch DENIES the call, so the outage is total (Bash, Edit, Write and Agent at once) and misattributed by construction: each denial names whichever hook matched, never the interpreter, and `Test-Path` on that file answers True -- which makes the corrupt-plugin-cache theory look confirmed rather than refuted. Written in shell rather than Python for the obvious reason: in the failure it reports, no Python hook can run. Probes `$0` -- the path the harness itself just handed the shell -- so it needs no path manipulation at all; deriving the directory would be the bug, since Git Bash `pwd -P` answers `/c/Users/...`, which a Windows `python3` cannot resolve, and a hook that normalised the path would report this outage on every healthy Windows machine. Silent when the interpreter is fine, and distinguishes "not on PATH" and "probe did not run" from the alias condition rather than asserting a cause it did not observe (ai-config#3624) |
