...
The source lives at `skills/session-lock/SKILL.md` in the same ai-config checkout as this wrapper.
If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/session-lock`, resolve the symlink target for this wrapper directory first, then read `../../skills/session-lock/SKILL.md` relative to that real directory.
