...
## Pattern 6: Working on Primary Checkout Instead of a Worktree
- **Mistake**: Committing directly to the main checkout or an existing branch on it, instead of isolating into a dedicated worktree.
- **Example**: wai session 2026-08-19: committed memory updates directly to `fix/quote-yaml-placeholders` on the primary ai-config checkout.
- **Canonical Rule**: `AGENTS.md` ("Worktree isolation"): always use a worktree for write/edit tasks so parallel sessions never clobber each other.
- **Fix**: Before any write session, create a worktree (`git worktree add`) on a feature branch. Never commit to the primary checkout.
