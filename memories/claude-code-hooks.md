...
(Some already resolved correctly by another spelling --- three non-test hooks elsewhere in `hooks/` --- `flag-unattributable-reviewer-request.py`, `no-misattributed-quote.py` and `no-unauthorized-merge.py`, only the first of which uses `_sibling()` --- already use `Path(__file__).resolve()` --- and were not touched.)
