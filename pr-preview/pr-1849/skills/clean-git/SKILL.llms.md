...
`INLINE` is the subset whose **worktree** `clean-worktrees` step 3 classified **Dead**, so intersect that mapping against the dead-worktree paths from step 1 before using it anywhere.
